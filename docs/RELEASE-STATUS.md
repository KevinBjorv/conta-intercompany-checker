...
- Local self-hosted n8n 2.39.6 on Windows/Node 24.13.1: 17 scenarios passed in a fresh isolated runtime on 2026-09-17. Actual HTTP nodes and separate synthetic credentials covered both company branches, four mapped accounts over ten requests, Retry-After seconds/date, exhausted retries, the shipped 30-second transport timeout, 401/403, malformed and empty responses, missing accounts, inconsistent movements and changed totals. Retry timing and the three-attempt limit were asserted. Separate probes verified exact raw-byte IDs/amounts and rejected redirects; the synthetic demo and invalid configuration also passed. See `release/n8n-smoke-results.json` for separate execution/report statuses and workflow hashes. This is CLI testing, not a Cloud or live Conta claim.
